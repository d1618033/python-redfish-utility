...
runRemote(){
  ilorest list --selector=SmartStorageArrayController. --url=$1 --user $2 --password $3
  ilorest logout
}

error(){
  echo "Usage:"
  echo        "remote: Get_Encrypt.sh ^<iLO url^> ^<iLO username^>  ^<iLO password^>"
  echo        "local:  Get_Encrypt.sh"
}
