...
class SmartNicCommand:
    ...
    def __init__(self):
        self.ident = {
            ...
            "smartnic --id <id> --update_os <image.iso>\n",
            ...
            if options.id is not None:
                ...
                if (
                    ...
                    for id in str_lst:
                        ...
                        elif options.system and not options.clearlog and not options.logs:
                            if id == info["Id"] and info["SystemType"] == "DPU":
                        ...
                        elif options.clearlog and options.system:
                            try:
                                for id in str_lst:
                                    if id == info["Id"]:
                ...
                if options.clearlog and options.system:
                    raise InvalidCommandLineError(
                        "No command --clearlog for smartnic\n"
                        "usage: smartnic --id <id> --clearlog"
                    )

                if options.system and not options.clearlog:
                    if info["SystemType"] == "DPU":
                        self.prettyprintinfo(info, fw_version)
    ...
    def definearguments(self, customparser):
        ...
        customparser.add_argument(
            dest="system",
            action="store_true",
            help="include this flag to add basic system information to the output",
            default=False,
        )
