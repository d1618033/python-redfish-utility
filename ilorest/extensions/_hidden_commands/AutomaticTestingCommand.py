...
class AutomaticTestingCommand:
    ...
    def getandsetautomatictesting(self, latestschema):
        ...
        self.rdmc.ui.printer(
            "*********************STARTING GET AND SET AUTOMATIC"
            " TESTING**********************\n"
        )
        self.rdmc.ui.printer(
            "***************************************************"
    ...
    def setautomatichelper(self, inputlist, selector, latestschema):
        ...
        for item in inputlist:
            listresults = self.auxcommands["get"].getworkerfunction(
                item, self.options, results=True
            )
            for getresults in listresults:
                if getresults and isinstance(getresults[next(iter(getresults))], dict):
                    ...
                    InvalidCommandLineError(
                        "Variable '%s' was not found in the "
                        "current selection.\n" % item
                    )

    @log_decor
    def getandinfoautomatictesting(
        self, item="", latestschema=False, getval=None, already_checked=[]
    ):
        ...
        """
        listdata = self.auxcommands["get"].getworkerfunction(
            item, self.options, results=True
        )
        if not listdata:
            self.rdmc.ui.warn("(GET) No list contents found for section: '%s'\n" % item)

        getdata = self.auxcommands["get"].getworkerfunction(
            item, self.options, uselist=True, results=True
        )
    ...
    def rawautomatictesting(self, local):
        ...
        try:
            ...
            self.auxcommands["rawget"].run(
                '"/rest/v1/systems/1/bios/Settings" -f TESTFILE'
            )
            testfile = open("TESTFILE")
            found = 0
        ...
        try:
            ...
            for line in testfile:
                ...
                if (
                    '"MySession":' in line
                    and '"MySession": false' not in line
...
#                            keep_going = True
    ...
    def accountautomatictesting(self):
        ...
        for i in range(5):
            ...
            for rp in enumerate(rand_privs):
                if rp[1] == "SystemRecoveryConfigPriv":
                    ...
                    rand_priv_str += str(priv_dict[rp[1]]) + ","
            acct_str = (
                "add " + randname + " " + randname + " testpassword " + rand_priv_str
            )
            try:
                ...
                if (
                    ...
                    try:
                        ...
                        if "rand_priv" in locals():
                            ...
                            errors.append(
                                "Unable to retrieve privileges for account '{}' "
        ...
        if errors:
            raise Exception(
                "The following errors occurred while testing accounts: %s" % errors
            )
    ...
    def federationautomatictesting(self):
        ...
        for fed in results:
            fed = self.rdmc.app.get_handler(
                fed[self.rdmc.app.typepath.defs.hrefstring], service=True, silent=True
            ).dict
    ...
    def ahsdiagtesting(self, local):
        ...
        if local and os.name != "nt":
            ...
            self.auxcommands["ahsdiag"].run(
                "--WriteMarkerPost --instance 1 --markervalue"
    ...
    def certificateautomatictesting(self):
        ...
        self.auxcommands["login"].run("")
        self.auxcommands["certificate"].run(
            "csr" + orgname + orgunit + commonname + country + state + city
        )
        ...
        for item in self.automatictesting_helper_fb(url_path):
            ...
            try:
                ...
                self.rdmc.ui.printer(
                    "iLO flagged an error while uploading the"
        ...
        if error_dict:
            try:
                ...
                for item in error_dict:
                    err.args = err.args + (
                        "Component: %s, Full String: %s" % (item, error_dict[item]),
                    )
                raise
    ...
    def sendtestautomatictesting(self):
        ...
        try:
            ...
            self.auxcommands["select"].run(
                self.rdmc.app.typepath.defs.managernetworkservicetype
            )
        except:
        ...
        try:
            ...
            self.auxcommands["set"].run(
                oem
                + "/RemoteSyslogServer=testserver.test.svrtest "
                + oem
                + "/RemoteSyslogEnabled=True --commit"
            )
        except:
            pass
    ...
    def directoryautomatictesting(self):
        ...
        self.auxcommands["directory"].run(
            "kerberos --serviceaddress test.account --port 1337 --realm " "testrealm"
        )
        self.auxcommands["directory"].run("ldap testusername testpassword --enable")
        self.auxcommands["directory"].run(
            "ldap --serviceaddress test2.account --addsearch autotestsearch,"
            "autotestsearch2"
        )
        ...
        if (
            "autotestsearch"
            in results["LDAP"]["LDAPService"]["SearchSettings"]["BaseDistinguishedNames"]
            and "autotestsearch2"
            in results["LDAP"]["LDAPService"]["SearchSettings"]["BaseDistinguishedNames"]
        ):
        ...
        if (
            results["LDAP"]["ServiceEnabled"]
            and not results["ActiveDirectory"]["ServiceEnabled"]
        ):
        ...
        self.auxcommands["directory"].run(
            'ldap --serviceaddress "" --disable --removesearch '
            "autotestsearch,autotestsearch2"
        )
        self.auxcommands["directory"].run(
            'ldap --removerole "dirgroupa test,dirgroupanother test"'
        )
        ...
        if not results["LDAP"]["ServiceEnabled"]:
            ...
            raise IloResponseError("")
        if (
            not results["ActiveDirectory"]["ServiceAddresses"][0]
            and not results["LDAP"]["ServiceAddresses"][0]
        ):
            self.rdmc.ui.printer("Validated Service addresses.\n")
        else:
    ...
    def serverclonecommandautomatictesting(self, local):
        ...
        url_path = (
            "http://infinitymaster.us.rdlabs.hpecorp.net:1051/automatic_testing/"
        ...
        try:
            ...
            error["unencrypted_clone"] = {
                "base_error": "An error occurred working with serverclone unencrypted save/load: {}".format(
                    excp
                ),
        ...
        try:
            ...
            line_str = (
                "save --encryption HPESecretAESKey1 --silent --nobios -f "
                + enc_clone_file_name
            )
            self.auxcommands["serverclone"].run(line_str)
        ...
        try:
            ...
            error["delete_cfs"] = "An error occurred deleting clone files: '%s'" % str(
                excp
            )

        if error:
            raise Exception(
                "The following exceptions occured in ServerClone:\n {}".format(error)
            )
    ...
    def deletecomtesting(self):
        ...
        if not self.rdmc.app.typepath.flagiften:
            self.rdmc.ui.printer(
                "Skipping delete component command, server is not gen10.\n"
            )
            return ReturnCodes.SUCCESS
    ...
    def uploadcomptesting(self):
        ...
        self.auxcommands["login"].run("")
        url_path = (
            "http://infinitymaster.us.rdlabs.hpecorp.net:1051/automatic_testing/components"
            "/COMPONENTS/"
        )

        if not self.rdmc.app.typepath.flagiften:
            self.rdmc.ui.printer(
                "Skipping upload component command, server is not gen10.\n"
            )
            return ReturnCodes.SUCCESS
    ...
    def listanddownloadcomptesting(self):
        ...
        if not self.rdmc.app.typepath.flagiften:
            self.rdmc.ui.printer(
                "Skipping download component command, server is not gen10.\n"
            )
            return ReturnCodes.SUCCESS
        ...
        for item in comps:
            ...
            self.rdmc.ui.printer(
                "Successfully downloaded '%s' from iLO "
                "Repository.\n" % item["Filename"]
            )
    ...
    def installsettesting(self):
        ...
        for installset in installsetlist:
            try:
                ...
                self.auxcommands["installset"].run(
                    "invoke --name=TestSet" + str(i) + " --cleartaskqueue"
                )
        ...
        for installset in installsetlist:
            try:
                ...
                self.rdmc.ui.printer(
                    "An error occured attempting to remove the "
                    "file: %s, logged: %s\n" % (installsetfile, excp)
                )
                continue
    ...
    def listcomptesting(self):
        ...
        if not self.rdmc.app.typepath.flagiften:
            self.rdmc.ui.printer(
                "Skipping list component command, server is not gen10.\n"
            )
            return ReturnCodes.SUCCESS
    ...
    def testsumcode(self):
        ...
        if not self.rdmc.app.typepath.flagiften:
            ...
            self.auxcommands["rawput"].run(
                "sut_registry_payload.json --getheaders --response"
                " --service --providerid=SUT-PROVIDER" + session_key
            )
            self.auxcommands["rawput"].run(
                "sut_schema.json --getheaders --response --service "
                "--providerid=SUT-PROVIDER" + session_key
            )
            self.auxcommands["rawput"].run(
                "sut_settings_schema.json --getheaders --response "
                "--service --providerid=SUT-PROVIDER" + session_key
            )
            self.auxcommands["rawput"].run(
            ...
            self.auxcommands["rawput"].run(
                "HPSUTSettings_rest.json --providerid=HPSUT-PROVIDER"
            )
            self.auxcommands["rawpatch"].run(
                "HPSUTSettings_rest.json --providerid=HPSUT-PROVIDER"
            )
            self.auxcommands["logout"].run("")
        ...
        if self.rdmc.app.typepath.flagiften:
            ...
            self.auxcommands["rawget"].run(
                "/redfish/v1/UpdateService/SoftwareInventory/ --expand"
            )
            self.auxcommands["rawget"].run(
                "/redfish/v1/updateService/installsets/ --expand"
            )
            ...
            self.auxcommands["uploadcomp"].run(
                "--component={0} --compsig={1} "
                "--forceupload".format(compname, compsigname_1)
            )

            self.auxcommands["rawget"].run(
                "/redfish/v1/updateService/ComponentRepository/ --expand"
            )
            self.auxcommands["rawget"].run(
                "/redfish/v1/Managers/1/EthernetInterfaces/ --expand"
            )
            self.auxcommands["logout"].run("")
            ...
            self.auxcommands["rawpost"].run(
                "session_payload_gen10.json --getheaders --service"
            )
            session_key = ""
            for client in self.rdmc.app._rmc_clients:
                session_key = client.get_authorization_key()
            ...
            self.auxcommands["rawput"].run(
                "sut_schema_gen10.json --getheaders --response "
                "--service --providerid=SUT-PROVIDER" + session_key
            )
            self.auxcommands["rawput"].run(
            ...
            self.auxcommands["rawput"].run(
                "HPSUT_redfish.json --providerid=HPSUT-PROVIDER"
            )
            self.auxcommands["rawput"].run(
                "HPSUTSettings_redfish.json --providerid=HPSUT-PROVIDER"
            )
            self.auxcommands["rawpatch"].run(
                "HPSUTSettings_redfish.json --providerid=HPSUT-PROVIDER"
            )
        ...
        self.auxcommands["rawget"].run("/redfish/v1/UpdateService/FirmwareInventory/")
        self.auxcommands["rawget"].run(
            "/redfish/v1/UpdateService/UpdateTaskQueue/ --expand"
        )
    ...
    def maintenancewindowautomatictesting(self):
        ...
        for entry_ in range(0, 10):
            ...
            self.rdmc.ui.printer(
                "Adding Maintenance Window: " + maintenancewindow_str + "\n"
            )
            ...
            self.rdmc.ui.printer(
                "Removing Maintenance Window: " + maintenancewindow_str + "\n"
            )
            self.auxcommands["maintenancewindow"].run(maintenancewindow_str)
    ...
    def serverinfoautomatictesting(self):
        ...
        info_string = (
            "--fans --processor --memory --thermals --power --system --showabsent"
        )
        self.rdmc.ui.printer(
            "\n*************************************************"
            "********************************\n"
    ...
    def ipprofilesautomatictesting(self):
        ...
        try:
            ...
            self.rdmc.ui.printer(
                "Skipping IP Profiles test, IP provider is unavailable.\n"
            )
    ...
    def fwintegritycheckautomatictesting(self):
        ...
        self.rdmc.ui.printer(
            "******************FIRMWARE INTEGRITY CHECK AUTOMATIC "
            "TESTING*********************\n"
        )
        self.rdmc.ui.printer(
            "*************************************************"
        ...
        if not self.rdmc.app.typepath.flagiften:
            self.rdmc.ui.printer(
                "Skipping firmware integrity check, only available on Gen 10."
            )
        else:
            self.auxcommands["login"].loginfunction("")
    ...
    def setbiosdefaultsautomatictesting(self):
        ...
        try:
            ...
            self.rdmc.ui.printer(
                repr(self.auxcommands["biosdefaults"].run("--manufacturingdefaults"))
            )
        except:
    ...
    def smartarrayautomatictesting(self):
        ...
        if content:
            tmp = self.auxcommands["smartarray"].selection_output(
                dict(controller=None), content
            )
        else:
    ...
    def serverclone_helper(self, filename=None, line=None):
        ...
        try:
            with open(filename, "r") as cf:
                if "--encryption" in line:
                    entries_list = [
                        (pos.start(), pos.end())
                        for pos in list(re.finditer(ending, path))
                    ]
                    encryption_key = None
                    data = json.loads(
                        Encryption().decrypt_file(cf.read(), encryption_key)
                    )
                else:
                    data = json.loads(cf.read())
        except Exception as excp:
            errors.append(
                "An error occurred opening the clone file '%s': %s" % (filename, excp)
            )
        ...
        try:
                ...
                if "--encryption" in line:
                    ...
                    cf.write(
                        Encryption().decrypt_file(
                            json.dumps(data, indent=2), encryption_key
                        )
                    )
                else:
            ...
            errors.append(
                "An error occurred writing the clone file '%s': %s" % (filename2, excp)
            )
    ...
    def automatictesting_helper_fb(self, url, components=False):
        ...
        try:
            ...
            read_url = read_url[
                read_url.find("<table>") : read_url.find("<table>")
                + read_url.find("</table>")
            ]
            while read_url.find("<tr>") > 0:
                current_str = read_url[prev_tr : prev_tr + read_url.find("</tr>")]
                current_str = current_str[
                    current_str.find("<a href") : current_str.find("<a href")
                    + current_str.find("</a>")
                ]
                current_str = current_str[
                    current_str.find('="')
                ...
                if (
                    current_str.split("/")[-1] in ignore_list
                    or current_str.split("/")[-2] in ignore_list
                ):
                    ...
                    for url in table_row_list:
                        ...
                        if last_element.count(".") > 2:
                            ...
                            if "compsig" in last_element:
                                if (
                                    components_dict.get(compname) is not None
                            ...
                            if "part" in last_element and "_" in last_element:
                                ...
                                if (
                                    split_compname in components_dict
                                    and url not in components_dict[split_compname]
                                ):
