    ...
    def run(self, line, help_disp=False):
        ...
        if self.rdmc.app.typepath.defs.isgen9:
            raise IncompatibleiLOVersionError(
                "GetInventory command is " "available only on iLO 5 node."
            )
        ...
        if options.inventory:
            try:
                results = self.rdmc.app.select(
                    selector="SoftwareInventoryCollection",
                    fltrvals=("Name", "Firmware Inventory Collection"),
                    path_refresh=True,
                )
            ...
            try:
                results = self.rdmc.app.select(
                    selector="SoftwareInventoryCollection",
                    fltrvals=("Name", "Software Inventory Collection"),
                    path_refresh=True,
                )
                collectiondata = next(iter(results)).dict
                members = self.rdmc.app.getcollectionmembers(
            ...
            try:
                results = self.rdmc.app.select(
                    selector="EthernetInterfaceCollection",
            ...
            try:
                results = self.rdmc.app.select(
                    selector="HpeiLODateTime.", path_refresh=True
                )
        ...
        if options.sut:
            ...
            if alldata.get("systems1"):
                try:
                    sutlocation = (
                        alldata.get("systems1").get("Oem").get("Hpe").get("Links")
                    )
                except:
                    if results:
                        sutlocation = next(iter(results)).dict.get("Links")
        ...
        if options.repo_data:
            try:
                results = self.rdmc.app.select(
                    selector="HpeComponentInstallSetCollection", path_refresh=True
                )
            ...
            try:
                results = self.rdmc.app.select(
                    selector="HpeComponentUpdateTaskQueueCollection", path_refresh=True
                )
                collectiondata = next(iter(results)).dict
                members = self.rdmc.app.getcollectionmembers(
                    collectiondata.get("@odata.id")
            ...
            if self.rdmc.app.getiloversion(skipschemas=True) >= 5.130:
                try:
                    results = self.rdmc.app.select(
                        selector="HpeMaintenanceWindowCollection", path_refresh=True
                    )
                    collectiondata = next(iter(results)).dict
                    members = self.rdmc.app.getcollectionmembers(
                        collectiondata.get("@odata.id")
