...
class Mapper(object):
    ...
    def get_multiple_attributes(self, data, attributes_list, mapping_table, output_as_json=False,
                                **resources):
        """
        ...
        if output_as_json:
            ...
            for attribute in attributes_list:
                output.update(self.get_single_attribute(data, attribute, mapping_table,
                                                        output_as_json=True, resources=resources))
        else:
            output = ""
            for attribute in attributes_list:
                output += "\n" + self.get_single_attribute(data, attribute, mapping_table,
                                                           resources=resources)
        return output

    @staticmethod
    def get_single_attribute(data, attribute_name, mapping_table, output_as_json=False,
                             resources=None):
        """
