...
class MapperRenderers(object):
    ...
    def calculate_total_capacity(**kwargs):
        ...
        if members:
            for member in members:
                capacity = Mapper.get_single_attribute(member, "Capacity",
    ...
    def calculate_total_pmem(**kwargs):
       ...
       """
        ...
        if members:
            for member in members:
                capacity = Mapper.get_single_attribute(member, "PmemSize",
                                                       MappingTable.device.value,
                                                       output_as_json=True)
    ...
    def calculate_total_volatile(**kwargs):
        ...
        if members:
            for member in members:
                capacity = Mapper.get_single_attribute(member, "VolatileSize",
                                                       MappingTable.device.value,
                                                       output_as_json=True)
    ...
    def find_dimm_ids(**kwargs):
        ...
        for interleave_set in interleave_sets:
            for pmem in memory_members:
                if PmemHelpers.compare_id(interleave_set.get("Memory").get("@odata.id"),
                                          pmem.get("@odata.id")):
                    location = Mapper.get_single_attribute(pmem, "Location",
                                                           MappingTable.device.value,
                                                           output_as_json=True)
    ...
    def calculate_task_pmem_size(**kwargs):
        ...
        memory_chunk_size = Mapper.get_single_attribute(task, "MemoryChunkSize",
        ...
        if memory_chunk_size is not None:
            ...
            total_capacity = Mapper.get_single_attribute(selected_members, "TotalCapacity",
            ...
            memory_chunk_size_percentage = Mapper.get_single_attribute(task,
                                                                       "MemoryChunkSizePercentage",
                                                                       MappingTable.tasks.value,
                                                                       output_as_json=True)
            memory_chunk_size_percentage = memory_chunk_size_percentage.get(
                "MemoryChunkSizePercentage", {}).get("Value", None)
    ...
    def calculate_chunk_volatile_size(**kwargs):
        ...
        volatile_size = total_capacity
        # finding memory chunk size
        memory_chunk_size = Mapper.get_single_attribute(chunk, "PmemSize",
                                                        MappingTable.delete_task.value,
                                                        output_as_json=True)
...
CONFIG_MAPPING_TABLE = {
    "PmemInterleaved":
        {
            "path": "",
            "compute": MapperRenderers.find_pmem_interleaved
        },
    "PmemSize":
        {
            "path": "/PersistentRegionSizeLimitMiB",
...
TASK_MAPPING_TABLE = {
    ...
    "Type":
        {
            "path": "/Payload/JsonBody/AddressRangeType"
        },
    "VolatileSize":
        {
            "path": "",
