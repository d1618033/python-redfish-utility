...
class ISToolCommand():
    ...
    def run(self, line, help_disp=False):
        ...
        try:
            ...
            romresults = self.rdmc.app.get_handler(self.rdmc.app.typepath.defs.systempath,
                                                   service=True, silent=True)
        ...
        if regresults.dict:
            ...
            for item in items:
                try:
                    ...
                    item = self.rdmc.app.get_handler(item[self.rdmc.app.typepath.defs.hrefstring],
                                                     service=True, silent=True)
                ...
                if schema:
                    ...
                    locationdict = self.rdmc.app.validationmanager.geturidict(item['Location'][0])\
                                                     if self.rdmc.app.validationmanager else None
                    extref = self.rdmc.app.get_handler(locationdict, service=True, silent=True) \
                                                                        if locationdict else None
            ...
            self.rdmc.ui.printer("The following attribute registries are in the registry \
                                                                                repository:\n")
            for item in reglist:
                self.rdmc.ui.printer("%s\n" % item)

        if foundreg:
            self.rdmc.ui.printer("The system attribute registry was found in the "
                                 "registry repository.\n")
        else:
            self.rdmc.ui.error("The system attribute registry was not found in "
