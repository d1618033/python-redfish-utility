...
class AHSdiagCommand:
        ...
        for _ in range(0, 3):  # channel loop for iLO
            ...
            try:
                for i in range(3):
                    ...
                    self.rdmc.ui.printer(
                        "attempting to register Marker Post..."
                        "failed {0} times\n".format(i + 1)
                    )
    ...
    def loadlib(self):
        ...
        try:
            if os.name == "nt":
                self.rdmc.ui.printer(
                    "Operation can be performed only on Unix based systems!\n"
                )
                raise InvalidCommandLineErrorOPTS("")
            else:
        ...
        try:
            ...
            if client:
                if not client.base_url == "blobstore://.":
                    raise InvalidCommandLineError(
                        "ahsdiag command " "available in local mode only.\n"
                    )
