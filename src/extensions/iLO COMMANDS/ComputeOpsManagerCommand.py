...
###
# -*- coding: utf-8 -*-
""" computeopsmanager Command for rdmc """
...
class ComputeOpsManagerCommand:
    """Main new command template class"""

    def __init__(self):
        self.ident = {
            "name": "computeopsmanager",
            "usage": "computeopsmanager\n\n",
            "description": "Run to enable your servers to be discovered, monitored and managed through Compute Ops Manager \n\t"
            "Example:\n\tcomputeopsmanager connect --activationkey <ACTIVATION KEY> or \n\t"
            "computeopsmanager connect --activationkey <ACTIVATION KEY> --proxy http://proxy.abc.com:8080 \n\t"
            "computeopsmanager disconnect or \n\t"
            "computeopsmanager status or \n\t"
            "computeopsmanager status -j\n",
            "summary": "Enables the server to be discovered, monitored and managed through Compute Ops Manager.",
            "aliases": ["computecloud"],
            "auxcommands": [],
        }
        self.cmdbase = None
        self.rdmc = None
            ...
            try:
                ...
                if path and body:
                    ...
                    self.rdmc.app.patch_handler(
                        path[0], body, service=False, silent=True
                    )
            except:
    ...
    def connect_cloud(self, activationkey):
        ...
        if status == "Connected":
            self.rdmc.ui.printer("Warning: Compute Ops Manager is already connected.\n")
        ...
        try:
            if path and body:
                self.rdmc.ui.printer(
                    "Connecting Compute Ops Manager...", verbose_override=True
                )
                self.rdmc.app.post_handler(path, body, service=False, silent=True)
        except:
            raise CloudConnectFailedError("Compute Ops Manager connection Failed\n")
        ...
        while True:
            ...
            if elapsed_time > allowed_seconds:
                ...
                raise CloudConnectTimeoutError(
                    "Compute Ops Manager connection timed out, Please check the "
                ...
                resp = self.rdmc.app.get_handler(path, service=False, silent=True)
                status = resp.dict["Oem"]["Hpe"]["CloudConnect"]["CloudConnectStatus"]
                # self.rdmc.ui.printer("Compute Ops Manager connection status is %s.\n" % status)
                if status == "Connected":
                    ...
                    if status == "Connected":
                        self.rdmc.ui.printer("\n")
                        self.rdmc.ui.printer(
                            "Compute Ops Manager connection is successful.\n"
                        )
                        break
                else:
                    self.rdmc.ui.printer("..")
    ...
    def disconnect_cloud(self):
        ...
        if cloud_status == "Connected":
            ...
            try:
                if path:
                    self.rdmc.ui.printer(
                        "Disconnecting Compute Ops Manager...\n", verbose_override=True
                    )
                    self.rdmc.app.post_handler(path, body)
            except:
                raise CloudConnectFailedError("Compute Ops Manager is not disconnected\n")
        else:
            self.rdmc.ui.printer("Warning: Compute Ops Manager is not at all connected.\n",
                                 verbose_override=True)
    ...
    def cloud_status(self, json=False):
        ...
        output = "------------------------------------------------\n"
        output += "Compute Ops Manager Status\n"
        output += "------------------------------------------------\n"
        output += "Compute Ops Manager Status : %s\n" % (
    ...
    def run(self, line, help_disp=False):
        ...
        if not self.rdmc.app.redfishinst:
            raise NoCurrentSessionEstablished(
                "Please login to iLO and retry the command"
            )

        ilo_ver = self.rdmc.app.getiloversion()
        if ilo_ver < 5.247:
            raise IncompatibleiLOVersionError(
                "Compute Ops Manager Feature is only available with iLO 5 version 2.47 "
                "or higher\n"
            )
        ...
        if options.command:
            if options.command.lower() == "connect":
                ...
                raise InvalidCommandLineError(
                    "%s is not a valid option for this "
        ...
        if resp.status == 200:
            ...
            if (
                ...
                raise CloudConnectFailedError(
                    "Compute Ops Manager is Disabled in this iLO\n"
                )
    ...
    def definearguments(self, customparser):
        ...
        connect_help = "To connect to Compute Ops Manager\n"
        # connect sub-parser
        connect_parser = subcommand_parser.add_parser(
            "connect",
            help=connect_help,
            description=connect_help
            + "\n\tExample:\n\tcomputeopsmanager connect --activationkey 123456789EFGA or "
            "\n\tcomputeopsmanager connect --activationkey 123456789EFGA --proxy http://proxy.abc.com:8080 or "
            "\n\tcomputeopsmanager connect --activationkey 123456789EFGA --proxy None",
        ...
        status_help = "To check the connection status\n"
