...
class EthernetCommand:
    ...
    def run(self, line, help_disp=False):
        ...
        if "default" in options.command.lower():
            ...
            if options.enable_enhanced_downloads or options.disable_enhanced_downloads:
                ...
                if ilo_ver < 5.263:
                    raise IncompatibleiLOVersionError("Enhance download required minimum "
                                                      "iLO Version iLO5.263")
                ...
                if options.enable_enhanced_downloads:
                    if results['Oem']['Hpe']['EnhancedDownloadPerformanceEnabled']:
                        self.rdmc.ui.printer("Enhanced Download Performance "
                                             "already enabled!! \n", verbose_override=True)
                        return_flag = False
                    else:
                        self.rdmc.ui.printer("Enabling enhanced "
                                             "downloads...\n", verbose_override=True)
                    ...
                    if not results['Oem']['Hpe']['EnhancedDownloadPerformanceEnabled']:
                        ...
                        self.rdmc.ui.printer("Disabling enhanced "
                                             "downloads...\n", verbose_override=True)
    ...
    def get_data(self, options):
        ...
        if self.rdmc.app.redfishinst.is_redfish:
            ...
            try:
                iface_results = self.rdmc.app.select(
                    selector="ethernetnetworkinterface."
                )
            except RisInstanceNotFoundError:
                self.rdmc.ui.printer(
                    "Type 'EthernetNetworkInterface.' not available.\n"
                )
        ...
        for inst in iface_results:
            try:
                ...
                self.rdmc.ui.printer(
                    "Missing instance data for type '%s' : '%s'"
                    % (inst.type, inst.path)
                )
    ...
    def load_ethernet_aux(self, _type, _path, ethernet_data):
        ...
        if "EthernetInterface" in _type:
            ...
            raise Exception(
                "Invalid type in management NIC load operation: '%s'" % _type
            )
        ...
        try:
            ...
            if (
                not curr_sel.dict["Oem"][self.rdmc.app.typepath.defs.oemhp][
                    "NICEnabled"
                ]
                and ethernet_data["Oem"][self.rdmc.app.typepath.defs.oemhp][
                    "NICEnabled"
                ]
            ):
                self.rdmc.app.patch_handler(
        ...
        try:
            ...
            # if DHCP Disable request but currently enabled
                ...
                not dhcpv4conf["DHCPEnabled"] and curr_sel.dict["DHCPv4"]["DHCPEnabled"]
            ):
                self.rdmc.app.patch_handler(
            ...
            try:
                if (
                    ...
                    and curr_sel.dict["Oem"][self.rdmc.app.typepath.defs.oemhp][
                        "DHCPv4"
                    ]["Enabled"]
                ):
        ...
        if oem_dhcpv4conf:
            flags["Oem"] = {
                self.rdmc.app.typepath.defs.oemhp: {"DHCPv4": oem_dhcpv4conf}
            }
        if oem_dhcpv6conf:
            flags["Oem"] = {
                self.rdmc.app.typepath.defs.oemhp: {"DHCPv6": oem_dhcpv6conf}
            }
        ...
        try:
            if "FrameSize" in list(ethernet_data.keys()):
                json_traversal_delete_empty(
                    data=ethernet_data, remove_list=["FrameSize"]
                )
        except (KeyError, NameError) as exp:
            errors.append("Unable to remove property %s.\n" % exp)
